#!/bin/bash
# One track's autonomous vote-opening run for the Reading Club — the child that
# run-auto-vote.sh (the cron entry) runs for senior, then for junior.
#   bash .bot/vote-track.sh                 # senior (default): the auto-vote skill
#   bash .bot/vote-track.sh --track=junior  # junior: the auto-vote-junior skill
#
# No time gate here — that lives in the driver — so a direct call opens the
# track's vote NOW (a supervised manual run; set AUTOVOTE_DATE to pick the
# date). The skills are idempotent: a track whose vote is already open (or
# whose reading is already published) is a no-op.
#
# The tracks never run concurrently: every track run (vote and publish) takes
# the one autopilot lock and WAITS for it, so the box (2 CPUs, ~4 GB) only ever
# hosts one headed browser + one claude session, and a manual run queues
# behind a cron run instead of colliding with it.
#
