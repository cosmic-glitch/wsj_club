...
4. **Propose the words and concepts, and get the user's sign-off before generating anything.** This is a required manual checkpoint — **do not write the JSON or generate the quiz until the user approves.** Based on your read of the article:
   - Pick your candidate **3 vocab words** and **2 concepts** (the default — a third only when it genuinely shouts to be included, hard cap 3; fewer is fine, even zero on a vocab-only day; the user can override the count) per the calibration above.
