...
- **Concepts**: the richer layer the article assumes you already understand — the things a definition alone won't fix. Examples: *the state attorney general, civil vs criminal cases, the labour force, student visas and green cards, ballistic vs cruise missiles, magazine depth, hyperscalers, private credit, quantitative easing, moral hazard, loss aversion, self-selection*.
  ...
  - **Optional `diagram` — rare, and it has to earn its place** (added 2026-08-29, the cancer-vaccine day, at the owner's prompt). A concept may carry `diagram: { src, alt, caption? }` pointing at a hand-authored SVG under `public/diagrams/<date>/<concept-slug>.svg`, rendered as a plain `<img>` **after** the `meaning` (read the mechanism, then check yourself against the picture). Use it **only when the idea is spatial** — something compared side by side, blocked, or passed along a chain — and the picture shows a mechanism the prose genuinely can't. That day qualified because three cards in a row asked the reader to hold a spatial picture in their head (a cell holding up fragments for inspection, a T cell meeting a stand-down signal, tumour DNA compared against healthy DNA). **Most days have nothing worth drawing, and a field that exists tends to get filled — the default is no diagram.** Never draw a bar chart of numbers the article states, and never illustrate a card the prose already lands. When you do draw: the SVG is `<img>`-embedded, so no page CSS reaches inside it — carry the brutalist palette yourself (white ground, `#0a0a0a` ink at `stroke-width: 3`, `#ffe600` for the one thing the eye should land on, monospace labels in caps). Author at a viewBox around **460 wide** and check it at **phone width first** (the card is ~300px there, so 11px viewBox text is about the floor); stack panels vertically rather than side by side. Reuse a visual across the day's cards when the ideas connect — the mRNA card ended on the same cell-with-one-yellow-fragment drawing the neoantigen card opened with, which is what tied the three together. Always write real `alt` text, and let the `caption` carry the takeaway rather than repeating the labels.
  ...
  "concepts": [
    {
      ...
      "diagram": {
        "src": "/diagrams/2026-06-09/hyperscalers.svg",
        "alt": "What the picture shows, for screen readers.",
        "caption": "One line under the figure — the takeaway, not a repeat of the labels."
      }
    }
  ],
  "quiz": [
...
- `diagram` is **optional and rare** — omit it on almost every day (see the concept calibration above for when it earns its place). `src` is the served path under `public/` (`/diagrams/YYYY-MM-DD/<concept-slug>.svg`), never a filesystem path. The SVG is committed with the day's other files.
