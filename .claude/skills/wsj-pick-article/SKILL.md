...
1. **Variety across days — this is the top rule.** The club should build *general* knowledge, so consecutive days should come from different domains. Before browsing, check what's already been covered:
   ...
   - Today's top pick should come from a domain **not used in the last few days**. Mentioning a runner-up from a recently-used domain is fine if it's exceptional, but say so.

2. **Stretched but not stressed.** The sweet spot is an article a sharp 13–16-year-old finds genuinely interesting and *slightly* above their level:
...
3. **Appropriate.** Skip stories centered on graphic violence, sexual content, or anything a parent would balk at handing a 13-year-old. War/conflict coverage is acceptable when the value is geopolitical understanding, not gore.
...
4. **Recommend.** Present to the user:
   - **One top pick** with its URL and a short case for it: the domain (and why it varies from recent days), the hook for a teenager, and the vocab/concept richness you spotted.
