...
5. **Persist the ranked field for the publish run.** `mkdir -p .bot/state` and write `.bot/state/${TODAY}-field.json` for the 7 balloted candidates, in rank order:
   ```json
