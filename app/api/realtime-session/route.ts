  ...
  const res = await fetch("https://api.openai.com/v1/realtime/client_secrets", {
    ...
    body: JSON.stringify({
      session: {
        ...
        // No tools: the quiz ends when the student clicks "End quiz". The tutor's
        // wrap-up tells them to press it (see buildInstructions, step 5) — an
        // earlier auto-end tool was unreliable, so the close is fully manual.
      },
    }),
  });
