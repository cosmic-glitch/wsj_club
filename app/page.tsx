...
import { getAllReadings, getAnnouncements, dateBig } from "@/lib/content";
...
export default function Home() {
  ...
  return (
    <>
      {/* The single site-wide announcement banner — the day's announcements
          (new features, reading guidance for today's article). Content lives
          in content/announcement.json; an empty messages list omits it. */}
      ...
      {readings.length === 0 ? (
        ...
        // A compact, table-like list: one row per day inside a single bordered
        // card, hairline dividers between days. Each row is the date + title on
        // a prominent line, then a small "action bar" of text links below. The
        // action bar WRAPS on narrow screens (it never scrolls horizontally),
        // which is why this beats a real <table> on mobile — a table would
        // force shared column widths and horizontal scroll on a phone.
        <ul className="divide-y divide-stone-200 overflow-hidden rounded-xl border border-stone-200 bg-white shadow-sm">
          {readings.map((r) => {
            // One link per source. Single-article days have one ("Article");
            // multi-article days have "Article 1", "Article 2", … each with
            // its own PDF.
            const sources =
            ...
            return (
              <li key={r.date} className="px-4 py-3 sm:px-5">
                {/* Date + title — the prominent line. The title is the anchor
                    (plain text, not a link); the date is a muted serif label
                    before it. */}
                ...
                {/* Compact action bar — text links separated by middots: Web
                    link · PDF · Handout · Voice quiz. The PDF is its own item
                    (not a "(PDF)" aside) so each link is a clear, separately
                    tappable target. */}
                <div className="mt-1 text-sm text-stone-600">
                  ...
                  {/* Login-gated + voiceQuiz-only; renders its own leading
                      middot when shown, nothing when signed out. */}
