...
// MUST match that script's. There is NO browser-speech fallback, so we only
// render the button when the clip actually exists on disk at build time — a
// missing clip means no button (never a dead one).
...
function audioSrcFor(date: string, term: string): string | undefined {
  const slug = slugify(term);
  const file = path.join(process.cwd(), "public", "audio", date, `${slug}.mp3`);
  return fs.existsSync(file) ? `/audio/${date}/${slug}.mp3` : undefined;
}
