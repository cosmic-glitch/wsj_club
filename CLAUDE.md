...
**Autopilot (senior, Economist-only):** the same day also runs unattended from the Hetzner box — `auto-vote` at 6:00am Pacific (scout → rank → open the vote → text the owner the ranked field) and `auto-publish` at 11:00am Pacific (tally → capture → author → glossary/audio → build → ship → announce to the club group). Publishing by hand before 11am makes the afternoon run a no-op. See **Autopilot** below.
