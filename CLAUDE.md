...
- **Models/voice** are env-overridable in case OpenAI renames them: `REALTIME_MODEL` (default `gpt-realtime-2`), `REALTIME_VOICE` (default `marin`), `REPORT_MODEL` (default `gpt-4o-mini`), `REALTIME_VAD_EAGERNESS` (default `low`). These exact values — including `semantic_vad`/`low` — were confirmed working against OpenAI's API this build.
