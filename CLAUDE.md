...
- **Handout/WordBank stay server components** — they fs-check `public/audio/` at build time (`lib/handout-audio.ts`); a dynamic render can't see those files on Vercel. `slugify()` there must match `scripts/gen-pronunciation.mjs`. The general recipe for personal/live bits on static pages: server-render the page, hydrate a small client leaf that fetches once (TodayTag/VotePoll/CompletedBy/WordBankList all follow it — never a fetch per row).
...
- The client's fixed `openingLine()` must mirror step 1 of `buildInstructions` (`lib/quiz-prompt.ts`) — the greeting skips the model and TTS entirely.
